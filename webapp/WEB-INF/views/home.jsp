            ...
            <form class="form-inline" action="${urlRoot}search" method="post">
                <div class="form-group">
                    <label for="fecha">Fecha: </label> <select id="fecha" name="fecha"
                                                               class="form-control">
                    <c:forEach items="${listaFechas}" var="unaFecha">
                        <c:choose>
                            <c:when test="${unaFecha eq fecha}">
                                <option value="${unaFecha}" selected>${unaFecha}</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${unaFecha}">${unaFecha}</option>
                            </c:otherwise>
                        </c:choose>
                ...
                <div class="col-xs-12 col-sm-6 col-md-3">
                    ...
                    <p>
                        <a class="btn btn-sm btn-primary"
                           href="detail/${pelicula.id}/${fecha}" role="button">Consulta
                            Horarios &raquo;</a>
                    </p>
                </div>
            </c:forEach>
